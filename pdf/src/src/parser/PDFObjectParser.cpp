...
namespace vrock::pdf
{
    PDFObjectParser::PDFObjectParser( )
        : BaseParser( "", 0 ), decryption_handler( std::make_shared<PDFNullSecurityHandler>( ) )
    {
    }

    PDFObjectParser::PDFObjectParser( std::string s )
        : BaseParser( std::move( s ), 0 ), decryption_handler( std::make_shared<PDFNullSecurityHandler>( ) )
    {
    }
